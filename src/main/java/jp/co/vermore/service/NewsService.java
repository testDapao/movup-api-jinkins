...
//        String now= DateUtil.dateToStringyyyy_MM_dd_HH_mm(new Date(System.currentTimeMillis()));
    ...
    public List<Category> getCategory() {
        ...
        for (News news1 : news) {
            String type =  news1.getType().toString();
        ...
        for(String s: strings){
            ...
            category.setCategoryType(s);
            category.setNums(map.get(s));
            String newsTitle = newsMapper.getNewsByTypeAndNew(s);
            category.setNewsTitle(newsTitle);
            categories.add(category);
        }
        return categories;
    }

    public Integer getCategoryMaxType() {
        return newsMapper.getCategoryMaxType();
    }

    public void deleteNewsByType(Byte tpe1) {
        int type = tpe1.intValue();
        newsMapper.deleteByType(type);

    }

    public List<Category> getCategoryByTitle(String title) {
        List<Category> category = this.getCategory();
        List<Category> categories = new ArrayList<>();
        for (Category category1 : category) {
            String categoryType = category1.getCategoryType();
            int i = Integer.parseInt(categoryType);
            if (i == 1) {
                categoryType = "EVENT";
            } else if (i == 2) {
                categoryType = "MOVEUP";
            ...
            if(categoryType.contains(title)){
